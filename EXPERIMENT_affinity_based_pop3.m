...
generation = 5000;
% regulator
proportional = [0 1 1.5 2 2.5 3 3.5 4 4.5];
integrator = 0.005;

filterwindow = 16;

for meranie=1:length(proportional)
...
for i=1:generation
    ...
    rest = ostrov.select('random',round((100-reqpopsize(i)*100-3)*proportional(meranie)));              % zvisok doplni nahodnym vyberom
...
eval(['popsize_pval_' num2str(proportional(meranie)*10) '_r_' num2str(runz) ' = mean(run,2);']);
save(['popsize_pval_' num2str(proportional(meranie)*10) '_r_' num2str(runz) '.mat'],['popsize_pval_' num2str(proportional(meranie)*10) '_r_' num2str(runz)]);
cd(retdir);
