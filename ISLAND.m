...
%             tmp=obj.extract('fitnes');
            obj.stats = [mean(obj.fitnes) max(obj.fitnes) min(obj.fitnes) median(obj.fitnes) mode(obj.fitnes) std(obj.fitnes) var(obj.fitnes) cov(obj.fitnes) getaff(obj.genes,obj.eps) obj.evaltime size(obj.genes,1) mean(pdist(obj.genes)) obj.bestknown.value];
...
%             for c=1:size(obj.population)
        ...
        function[varargout]=blobulate(varargin)
            obj = varargin{1};
            num = varargin{2};
