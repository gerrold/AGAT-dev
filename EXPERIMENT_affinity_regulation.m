...
ostrov = ISLAND;
ostrov = ostrov.set('space','homo',-520,520,variables,'fitfunc','eggholder','popsize',100);
ostrov = ostrov.seed();
...
for i=1:generation
    ...
    mutmiera(i) = ostrov.statfilt('affinity','mean',filterwindow);
    ...
    rest = rest.toolbox26('mutx',action(i),rest.space);   % to iste
    rest = rest.toolbox26('muta',action(i),rest.space(2,:) .* 0.01, rest.space);    % aditivna mutacia o 1% zo space hodnoty
...
eval(['affreg_lvl_' num2str(required_aff*100) '_r_' num2str(runz) ' = mean(run,2);']);
save(['affreg_lvl_' num2str(required_aff*100) '_r_' num2str(runz) '.mat'],['affreg_lvl_' num2str(required_aff*100) '_r_' num2str(runz)]);
cd(retdir);
